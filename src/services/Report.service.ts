...
async getProjectTaskBreakdown(projectId: number, userId: number) {
  ...
  async getProjectTeamAllocation(projectId: number, userId: number) {
    ...
    const members = await db
      ...
      .select(["pm.userId", "u.username", "u.email", "pm.role"])
    ...
    for (const member of members) {
      const assignedTasks = await db
        ...
        .select([
          (eb) => eb.fn.count<number>("ta.id").as("total"),
      ...
      teamWorkload.push({
        ...member,
        taskCount: Number(assignedTasks.total),
        completedCount: Number(assignedTasks.completed)
      });
    }
  ...
  async getTaskCompletionTrend(userId: number, days: number = 30, projectId?: number) {
    ...
    let query = db
      ...
      .leftJoin("task_assignments as ta", "ta.taskId", "t.id")
      .select([
        (eb) => eb.fn("DATE", "t.completedAt").as("date"),
        (eb) => eb.fn.count<number>("t.id").as("count")
      ])
      .where(eb => eb.or([
        eb("t.ownerId", "=", userId),
      ...
      .where("t.completedAt", "<=", endDate)
      .groupBy((eb) => eb.fn("DATE", "t.completedAt"))
      .orderBy((eb) => eb.fn("DATE", "t.completedAt"), "asc");
    ...
    const completionData = await query.execute();
